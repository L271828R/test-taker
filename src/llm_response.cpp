...
std::vector<GameQuestionBlock>
    ...
    std::vector<GameQuestionBlock> result;
    std::istringstream ss(response);
    std::string line, question, correct, wrong;
    auto flush = [&]() {
        ...
        question.clear(); correct.clear(); wrong.clear();
    };
    while (std::getline(ss, line)) {
        if (!line.empty() && line.back() == '\r') line.pop_back();
        if (line == "---") { flush(); continue; }
        if      (line.rfind("QUESTION:", 0) == 0) question = trim(line.substr(9));
        else if (line.rfind("CORRECT:",  0) == 0) correct  = trim(line.substr(8));
